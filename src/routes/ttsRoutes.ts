...
router.post('/tts', async (req: Request, res: Response) => {
  ...
  if (fileName !== undefined) {
    if (typeof fileName !== 'string') {
      res.status(400).json({ error: '`fileName` must be a string when provided.' });
      return;
    }

    if (!fileName.trim()) {
      res.status(400).json({ error: '`fileName` cannot be empty when provided.' });
      return;
    }
  }
  ...
  try {
    ...
    const requestedFileName = typeof fileName === 'string' ? fileName.trim() : undefined;
    const audio = await createAudioFile(normalizedText, selectedVoice, requestedFileName);
